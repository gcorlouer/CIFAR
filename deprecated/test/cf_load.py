...
def chan2DK(dfelec, electrode_name):
    DK = dfelec['ROI_DK'].loc[dfelec['electrode_name']== electrode_name]
    DK = DK.values
...
def picks2brodman(dfelec, picks):
    ROIs = []
    for chan in picks:
...
def picks2DK(dfelec, picks):
    ROI_DK = []
    for chan in picks:
        DK = chan2DK(dfelec, chan)
        ROI_DK.append(DK)
    return ROI_DK
...
class Subject:
    ...
    def ROI_DK(self, picks):
        dfelec = self.dfelec()
        ROI_DK = picks2DK(dfelec, picks)
        return ROI_DK
    
    def ROIs(self, picks):
        dfelec = self.dfelec()
        ROIs = picks2brodman(dfelec, picks)
        return ROIs

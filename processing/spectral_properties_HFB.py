...
def plot_psd(psd, freqs, average=True):
    (nchan, nfreq) = psd.shape
    psd = np.log(psd)
    bands = [4, 8, 16]
    bands_name = ['delta', 'theta', 'alpha']
    if average is True:
        psd = np.mean(psd, axis=0)
        plt.plot(freqs, psd)
        plt.xscale('log')
        ...
        for i in range(len(bands)):
            plt.axvline(x=bands[i], label = bands_name[i], color = 'k')
    else:
        ...
        for i in range(len(bands)):
            plt.axvline(x=bands[i], label = bands_name[i], color = 'k')
    plt.title('Power spectral density of visually responsive HFB envelope')
...
plot_psd(psd,freqs, average = True,)
